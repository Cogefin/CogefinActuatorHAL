...
void setLed(byte red, byte green, byte blue) {
  JsonDocument doc;
  doc["id"] = led_num;
  doc["type"] = LED_TYPE_COLOR_LED;
  doc["time"] = millis();
  doc["command"] = LED_COMMAND_SET_RGB;
  doc["paramSize"] = 1;
  doc["param"][0]["red"] = red;
...
void setup_LED(void) {
  ...
  updateDeviceTable(deviceCounter, LED_TYPE_COLOR_LED, &akCommon);
  led_num = deviceCounter;
  deviceCounter++;
  Serial.print("anode/cathod common color led : device No. ");Serial.println(led_num);
}
