...
void pmw_off(void) {
  JsonDocument doc;
  doc["id"] = pmw_num;
  doc["type"] = PMW_TYPE_SIMPLE;
  doc["time"] = millis();
  doc["command"] = PMW_COMMAND_OFF;
  doc["paramSize"] = 0;
...
void pmw_set(uint8_t val) {
  JsonDocument doc;
  doc["id"] = pmw_num;
  doc["type"] = PMW_TYPE_SIMPLE;
  doc["time"] = millis();
  doc["command"] = PMW_COMMAND_SET;
  doc["paramSize"] = 1;
...
void checkReturnValue(PmwFunctionReturnValue val, uint8_t funcType) {
  char buff[BUFFER_SIZE];
  memset(buff,0,BUFFER_SIZE);
  if (val == PMW_FUNCTION_UNSUPPORTED) {
    Serial.println("");
    sprintf(buff,"executed function \"%s\" is unsupported.",functionName[funcType].c_str());
    Serial.println(buff);
    return;
  }
  if (val == PMW_FUNCTION_FAIL) {
    Serial.println("");
...
void setup_PMW(void) {
  PmwFunctionReturnValue value;

  value = pmw.begin();
  checkReturnValue(value,FUNCTION_BEGIN);

  updateDeviceTable(deviceCounter, PMW_TYPE_SIMPLE, &pmw);
  pmw_num = deviceCounter;
  deviceCounter++;
  Serial.print("Simple PMW device : device No. ");Serial.println(pmw_num);
}
...
void test1() {
  ...
  Serial.println("set level of PMW to 15, 35, 55, ... 255 (3sec)");
  waitForStart();
